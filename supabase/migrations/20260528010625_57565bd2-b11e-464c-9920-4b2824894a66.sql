...
WITH CHECK (
  ...
  AND (
    last_spin_at IS NULL
    OR last_spin_at >= COALESCE((SELECT gs.last_spin_at FROM public.game_state gs WHERE gs.user_id = auth.uid()), '-infinity'::timestamptz)
  )
  -- collected_cards: append-only (can grow but cannot lose any existing card)
  ...
  AND NOT EXISTS (
    ...
    WHERE t.c <> ALL (COALESCE((SELECT gs.collected_cards FROM public.game_state gs WHERE gs.user_id = auth.uid()), ARRAY[]::text[]))
  )
  -- unlocked_dice_tiers: cannot grow AND cannot shrink (immutable via this path)
  ...
  AND NOT EXISTS (
    ...
    WHERE t.new_tier <> ALL (COALESCE((SELECT gs.unlocked_dice_tiers FROM public.game_state gs WHERE gs.user_id = auth.uid()), ARRAY[]::text[]))
  )
  AND NOT EXISTS (
    SELECT 1 FROM unnest(COALESCE((SELECT gs.unlocked_dice_tiers FROM public.game_state gs WHERE gs.user_id = auth.uid()), ARRAY[]::text[])) t(old_tier)
    WHERE t.old_tier <> ALL (game_state.unlocked_dice_tiers)
  )
  -- unlocked_monsters: cannot grow AND cannot shrink (immutable via this path)
  ...
  AND NOT EXISTS (
    ...
    WHERE t.m <> ALL (COALESCE((SELECT gs.unlocked_monsters FROM public.game_state gs WHERE gs.user_id = auth.uid()), ARRAY[]::text[]))
  )
  AND NOT EXISTS (
    SELECT 1 FROM unnest(COALESCE((SELECT gs.unlocked_monsters FROM public.game_state gs WHERE gs.user_id = auth.uid()), ARRAY[]::text[])) t(old_m)
    WHERE t.old_m <> ALL (game_state.unlocked_monsters)
  )
);